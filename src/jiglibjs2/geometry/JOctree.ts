...
export class JOctree {
  ...
  buildOctree(maxTrianglesPerCell: number, minCellSize: number) {
    ...
    while (cellsToProcess.length != 0) {
      ...
      if (
        this._cells[cellIndex].triangleIndices.length <= maxTrianglesPerCell ||
        this._cells[cellIndex].AABox.getRadiusAboutCentre() < minCellSize
      ) {
        continue;
      }
      for (i = 0; i < OctreeCell.NUM_CHILDREN; i++) {
        ...
        this._cells.push(
          new OctreeCell(this.createAABox(this._cells[cellIndex].AABox, i))
        );
        ...
        for (var j = 0; j < numTriangles; j++) {
          ...
          if (
            this.doesTriangleIntersectCell(this._triangles[iTri], childCell)
          ) {
    ...
    while (cellsToProcess.length != 0) {
      ...
      if (cell.isLeaf()) {
        ...
        for (var i = 0; i < OctreeCell.NUM_CHILDREN; i++) {
          if (
            cell.childCellIndices[i] >= 0 &&
            cell.childCellIndices[i] < this._cells.length
          ) {
  ...
  createAABox(aabb: JAABox, _id) {
    ...
    result.minPos = aabb.minPos.add(
      new Vector3D(offset.x * dims.x, offset.y * dims.y, offset.z * dims.z)
    );
