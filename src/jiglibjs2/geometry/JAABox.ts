...
export class JAABox {
  ...
  addSphere(sphere: JSphere) {
    ...
    this.minPos.x =
      sphere.get_currentState().position.x - sphere.get_radius() - 1;
    //}
    ...
    this.minPos.y =
      sphere.get_currentState().position.y - sphere.get_radius() - 1;
    //}
    ...
    this.minPos.z =
      sphere.get_currentState().position.z - sphere.get_radius() - 1;
    //}
  ...
  segmentAABoxOverlap(seg) {
    ...
    var pt, minPosArr, maxPosArr, p0, p1, faceOffsets: any;
    ...
    for (i = 0; i < 3; i++) {
      ...
      for (iFace = 0; iFace < 2; iFace++) {
        ...
        frac = -1;
        if (dist0 * dist1 < -tiny) frac = -dist0 / (dist1 - dist0);
        else if (Math.abs(dist0) < tiny) frac = 0;
        else if (Math.abs(dist1) < tiny) frac = 1;

        if (frac >= 0) {
          pt = JNumber3D.toArray(seg.getPoint(frac));
          if (
            pt[jDir] > minPosArr[jDir] - tiny &&
