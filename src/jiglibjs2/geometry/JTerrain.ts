...
export class JTerrain extends RigidBody {
  ...
  getSurfacePosByIndex(i, j) {
    return new Vector3D(
      this._terrain.minW + i * this._terrain.dw,
      this.getHeightByIndex(i, j),
      this._terrain.minH + j * this._terrain.dh
    );
  }
  ...
  getHeightAndNormalByPoint(point) {
    ...
    iFrac =
      1 - (w - (i0 * this._terrain.dw + this._terrain.minW)) / this._terrain.dw;
    jFrac =
      (h - (j0 * this._terrain.dh + this._terrain.minH)) / this._terrain.dh;
    iFrac = JMath3D.getLimiteNumber(iFrac, 0, 1);
    jFrac = JMath3D.getLimiteNumber(jFrac, 0, 1);

    // yUp for lite
    h00 = this._yUp
      ? this._terrain.heights[i0][j0]
      : -this._terrain.heights[i0][j0];
    h01 = this._yUp
      ? this._terrain.heights[i0][j1]
      : -this._terrain.heights[i0][j1];
    h10 = this._yUp
      ? this._terrain.heights[i1][j0]
      : -this._terrain.heights[i1][j0];
    h11 = this._yUp
      ? this._terrain.heights[i1][j1]
      : -this._terrain.heights[i1][j1];

    var obj = new TerrainData();
    var plane;
    if (iFrac < jFrac || i0 == i1 || j0 == j1) {
      obj.normal = new Vector3D(0, h11 - h10, this._terrain.dh).crossProduct(
        new Vector3D(this._terrain.dw, h11 - h01, 0)
      );
      // yUp for lite
      if (!this._yUp) obj.normal.negate();
      obj.normal.normalize();

      plane = new PlaneData();
      plane.setWithNormal(
        new Vector3D(
          i1 * this._terrain.dw + this._terrain.minW,
          h11,
          j1 * this._terrain.dh + this._terrain.minH
        ),
        obj.normal
      );
      obj.height = plane.pointPlaneDistance(point);
    } else {
      obj.normal = new Vector3D(0, h01 - h00, this._terrain.dh).crossProduct(
        new Vector3D(this._terrain.dw, h10 - h00, 0)
      );
      // yUp for lite
      if (!this._yUp) obj.normal.negate();
      obj.normal.normalize();
      ...
      plane.setWithNormal(
        new Vector3D(
          i0 * this._terrain.dw + this._terrain.minW,
          h00,
          j0 * this._terrain.dh + this._terrain.minH
        ),
        obj.normal
      );
      obj.height = plane.pointPlaneDistance(point);
  ...
  getSurfacePosByPoint(point) {
    return new Vector3D(
      point.x,
      this.getHeightAndNormalByPoint(point).height,
      point.z
    );
  }
