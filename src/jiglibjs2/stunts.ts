...
import { mat3, mat4, vec3 } from './glMatrix';
import { cars, defaultTrack, IModel, materials, models, terrainModels, trackModels, wheels } from './models';
import { PhysicsSystem } from './physics/PhysicsSystem';
import { ISkin3D } from './physics/RigidBody';
import { JCar } from './vehicles/JCar';
...
function physicalModel(model) {
  ...
  var indices: TriangleVertexIndices[] = [];
  ...
  var skin: ISkin3D = { transform: null, vertices: null, indices: null };
  skin.vertices = vertices;
  skin.indices = indices;

  var pos = vec(trackPosition);
  var rot = new Matrix3D();
  return new JTriangleMesh(skin, pos, rot, 200, 5 * worldScale);
}
...
function initCar(carModel) {
  ...
  car.steerRate = 3;
  car.driveTorque = 2000 * worldScale * worldScale;
  ...
  car
    .get_chassis()
    .set_sideLengths(
      vec([xmax - xmin, (ymax - ymin) * carScaleY, zmax - zmin])
    );

  var startPosition = [
    (startInfo.x + 1) * edgeLength,
    ymin +
      (ymax - ymin) * (1 - carScaleY) +
      wheelData[0].radius * wheelScale +
      0.1,
  ...
  // Initialise meshes
  var followDistance =
    vec3.length([xmax - xmin, ymax - ymin, zmax - zmin]) * 1.2;
  ...
  car.chassisMesh = initMesh(
    mergeModels([{ trans: trans, model: cars[carModel] }])
  );
...
function initPhysics() {
  jl = PhysicsSystem.getInstance();
  jl.setCollisionSystem(
    true,
    trackPosition[0],
    trackPosition[1],
    trackPosition[2],
...
function initGL(canvas: HTMLCanvasElement) {
  ...
  if (supported.indexOf('OES_standard_derivatives') < 0) {
    notify(
      'OES_standard_derivatives not supported! Disabling Moir辿 reduction.'
    );
    /*gl.*/ antiMoire = '';
  }
}
...
function initMesh(model: IModel) {
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(flatMaterialInfo),
    gl.STATIC_DRAW
  );
...
function renderMesh(mesh: Mesh) {
  ...
  gl.bindBuffer(gl.ARRAY_BUFFER, mesh.vertexColour);
  gl.vertexAttribPointer(
    shaderProgram.vertexColour,
    mesh.vertexColour.itemSize,
    gl.FLOAT,
    false,
    0,
    0
  ...
  gl.bindBuffer(gl.ARRAY_BUFFER, mesh.vertexMaterialInfo);
  gl.vertexAttribPointer(
    shaderProgram.vertexMaterialInfo,
    mesh.vertexMaterialInfo.itemSize,
    gl.FLOAT,
    false,
    0,
    0
...
function updateScene() {
  ...
  vec3.scale(
    camPos,
    Math.max(
      camera.distances.near,
      Math.min(camera.distances.far, vec3.length(camPos))
    )
  );
  ...
  mat4.multiply(
    cameraProjection,
    mat4.lookAt(camera.position, tempPos, [0, 1, 0])
  );
  ...
  for (var i = 0; i < carWheels.length; i++) {
    ...
    mat4.scale(worldProjection, [
      wheelData[i].width * wheelScale,
      wheelData[i].radius * wheelScale,
      wheelData[i].radius * wheelScale,
    ]);
  ...
  if (!t0) {
    ...
    car.setCar(
      car.maxSteerAngle,
      car.steerRate / Math.max(1, (speed - 10) * 0.5),
      car.driveTorque
    );
    for (i = 0; i < n; i++) {
      jl.integrate(ms);
...
function buildTrack(codes: number[]) {
        ...
        default:
          switch (terrainData[y][x]) {
            ...
            case 0x08:
              ...
              break;
            case 0x09:
              if (map09[c] != undefined) {
                c = map09[c];
              }
              break;
            case 0x0a:
              if (map0A[c] != undefined) {
                c = map0A[c];
              }
              break;
          }
  ...
  for (y = 0; y < terrainData.length; y++) {
    for (x = 0; x < terrainData[y].length; x++) {
      ...
      var r = trackData[y][x];
      if (
        ((t == 0x07 || t == 0x09) && (r == 0x04 || r == 0x0e || r == 0x18)) ||
  ...
  for (var i = 0; i < terrainItems.length; i++) {
    ...
    if (model[0][0] == 'lakc') {
      physicalModels.push({
        trans: makeTrans(item.x, item.y, item.elevated, orientation + 2),
        model: 'lakc',
      });
  ...
  for (i = 0; i < trackItems.length; i++) {
    ...
    if (
      isElement(
        [
          0x01, 0x86, 0x93, 0xb3, 0x87, 0x94, 0xb4, 0x88, 0x95, 0xb5, 0x89,
          0x96,
        ],
        item.id
      )
