...
import { CollisionSystemAbstract } from '../collision/CollisionSystemAbstract';
import { CollisionSystemBrute } from '../collision/CollisionSystemBrute';
import { CollisionSystemGrid } from '../collision/CollisionSystemGrid';
import { ContactData } from '../data/ContactData';
import { Vector3D } from '../geom/Vector3D';
import { JMath3D } from '../math/JMath3D';
import { JNumber3D } from '../math/JNumber3D';
import { BodyPair } from './BodyPair';
import { CachedImpulse } from './CachedImpulse';
...
export class PhysicsSystem {
  ...
  setCollisionSystem(
    ...
    if (collisionSystemGrid) {
      this._collisionSystem = new CollisionSystemGrid(
        sx,
        sy,
        sz,
        nx,
  ...
  setGravity(gravity) {
    ...
    if (
      this._gravity.x == this._gravity.y &&
      this._gravity.y == this._gravity.z
    )
      this._gravityAxis = -1;
    ...
    if (
      Math.abs(this._gravity.z) >
      Math.abs(JNumber3D.toArray(this._gravity)[this._gravityAxis])
    )
      this._gravityAxis = 2;

    // do update only when dirty, faster than call every time in step
    for (
      var _bodies_i = 0, _bodies_l = this._bodies.length, body;
      _bodies_i < _bodies_l && (body = this._bodies[_bodies_i]);
      _bodies_i++
    )
  ...
  removeAllConstraints() {
    for (
      var _constraints_i = 0,
        _constraints_l = this._constraints.length,
        constraint;
      _constraints_i < _constraints_l &&
      (constraint = this._constraints[_constraints_i]);
      _constraints_i++
    ) {
  ...
  removeAllControllers() {
    for (
      var _controllers_i = 0,
        _controllers_l = this._controllers.length,
        controller;
      _controllers_i < _controllers_l &&
      (controller = this._controllers[_controllers_i]);
      _controllers_i++
    ) {
  ...
  moreCollPtPenetration(info0, info1) {
    if (info0.initialPenetration < info1.initialPenetration) return 1;
    else if (info0.initialPenetration > info1.initialPenetration) return -1;
    else return 0;
  }
  ...
  preProcessCollisionFast(collision, dt) {
    ...
    var timescale = JConfig.numPenetrationRelaxationTimesteps * dt,
      ...
      allowedPenetration = JConfig.allowedPenetration;
    var ptInfo;
    var collision_pointInfo = collision.pointInfo;

    if (collision_pointInfo.length > 3) {
      collision_pointInfo = collision_pointInfo.sort(
        this.moreCollPtPenetration
      );
    ...
    for (let ptInfo of collision_pointInfo) {
      if (!body0.get_movable()) {
        ...
        ptInfo.denominator =
          body0.get_invMass() + N.dotProduct(tempV.crossProduct(ptInfo.r0));
      }
      ...
      if (body1 && body1.get_movable()) {
        ...
        ptInfo.denominator +=
          body1.get_invMass() + N.dotProduct(tempV.crossProduct(ptInfo.r1));
      }

      if (ptInfo.denominator < tiny) ptInfo.denominator = tiny;

      if (ptInfo.initialPenetration > allowedPenetration) {
        ptInfo.minSeparationVel =
          (ptInfo.initialPenetration - allowedPenetration) / timescale;
      } else {
        approachScale =
          (-0.1 * (ptInfo.initialPenetration - allowedPenetration)) /
          allowedPenetration;
        ...
        ptInfo.minSeparationVel =
          (approachScale * (ptInfo.initialPenetration - allowedPenetration)) /
          dt;
      }
  ...
  preProcessCollisionAccumulated(collision: CollisionInfo, dt: number) {
    ...
    for (let ptInfo of collision_pointInfo) {
      ...
      for (let cachedContact of this._cachedContacts) {
        if (
          !(
            bp.body0 == cachedContact.pair.body0 &&
            bp.body1 == cachedContact.pair.body1
          )
        )
          continue;

        var distSq =
          cachedContact.pair.body0 == body0
            ? cachedContact.pair.r.subtract(ptInfo.r0).get_lengthSquared()
            : cachedContact.pair.r.subtract(ptInfo.r1).get_lengthSquared();

        if (distSq < bestDistSq) {
          bestDistSq = distSq;
          ptInfo.accumulatedNormalImpulse = cachedContact.impulse.normalImpulse;
          ptInfo.accumulatedNormalImpulseAux =
            cachedContact.impulse.normalImpulseAux;
          ptInfo.accumulatedFrictionImpulse =
            cachedContact.impulse.frictionImpulse;

          if (cachedContact.pair.body0 != body0)
            ptInfo.accumulatedFrictionImpulse = JNumber3D.getScaleVector(
              ptInfo.accumulatedFrictionImpulse,
              -1
            );
        }
      ...
      if (ptInfo.accumulatedNormalImpulse != 0) {
        var impulse = JNumber3D.getScaleVector(
          N,
          ptInfo.accumulatedNormalImpulse
        );
        impulse = impulse.add(ptInfo.accumulatedFrictionImpulse);
        body0.applyBodyWorldImpulse(impulse, ptInfo.r0, false);
        if (body1)
          body1.applyBodyWorldImpulse(
            JNumber3D.getScaleVector(impulse, -1),
            ptInfo.r1,
            false
          );
      }
      ...
      if (ptInfo.accumulatedNormalImpulseAux != 0) {
        impulse = JNumber3D.getScaleVector(
          N,
          ptInfo.accumulatedNormalImpulseAux
        );
        body0.applyBodyWorldImpulseAux(impulse, ptInfo.r0, false);
        if (body1)
          body1.applyBodyWorldImpulseAux(
            JNumber3D.getScaleVector(impulse, -1),
            ptInfo.r1,
            false
          );
  ...
  processCollisionNormal(collision: CollisionInfo, dt: number) {
    ...
    for (let ptInfo of collision_pointInfo) {
      ...
      if (deltaVel <= this._minVelForProcessing) continue;
      ...
      if (tangent_speed > this._minVelForProcessing) {
        ...
        if (body0.get_movable()) {
          ...
          tempV = body0.get_worldInvInertia().transformVector(tempV);
          denominator =
            body0.get_invMass() + T.dotProduct(tempV.crossProduct(ptInfo.r0));
        }
        ...
        if (body1 && body1.get_movable()) {
          ...
          tempV = body1.get_worldInvInertia().transformVector(tempV);
          denominator +=
            body1.get_invMass() + T.dotProduct(tempV.crossProduct(ptInfo.r1));
        }
  ...
  processCollisionAccumulated(collision, dt) {
    ...
    var N = collision.dirToBody,
      ...
      tangent_vel,
      frictionImpulseVec,
      origAccumulatedFrictionImpulse,
    ...
    for (let ptInfo of collision_pointInfo) {
      ...
      if (ptInfo.minSeparationVel < 0) deltaVel += ptInfo.minSeparationVel;

      if (Math.abs(deltaVel) > this._minVelForProcessing) {
        normalImpulse = deltaVel / ptInfo.denominator;
        var origAccumulatedNormalImpulse = ptInfo.accumulatedNormalImpulse;
        ptInfo.accumulatedNormalImpulse = Math.max(
          ptInfo.accumulatedNormalImpulse + normalImpulse,
          0
        );
        var actualImpulse =
          ptInfo.accumulatedNormalImpulse - origAccumulatedNormalImpulse;
      ...
      if (Math.abs(deltaVel) > this._minVelForProcessing) {
        ...
        ptInfo.accumulatedNormalImpulseAux = Math.max(
          ptInfo.accumulatedNormalImpulseAux + normalImpulse,
          0
        );
        actualImpulse =
          ptInfo.accumulatedNormalImpulseAux - origAccumulatedNormalImpulse;
      ...
      if (ptInfo.accumulatedNormalImpulse > 0) {
        ...
        tangent_vel = VR.subtract(
          JNumber3D.getScaleVector(N, VR.dotProduct(N))
        );
        tangent_speed = tangent_vel.get_length();

        if (tangent_speed > this._minVelForProcessing) {
          let T = JNumber3D.getScaleVector(
            JNumber3D.getDivideVector(tangent_vel, tangent_speed),
            -1
          );
          denominator = 0;
          if (body0.get_movable()) {
          ...
          if (denominator > tiny) {
            ...
            origAccumulatedFrictionImpulse =
              ptInfo.accumulatedFrictionImpulse.clone();
            ptInfo.accumulatedFrictionImpulse =
              ptInfo.accumulatedFrictionImpulse.add(frictionImpulseVec);

            AFIMag = ptInfo.accumulatedFrictionImpulse.get_length();
            maxAllowedAFIMag =
              collision.mat.friction * ptInfo.accumulatedNormalImpulse;

            if (AFIMag > tiny && AFIMag > maxAllowedAFIMag)
              ptInfo.accumulatedFrictionImpulse = JNumber3D.getScaleVector(
                ptInfo.accumulatedFrictionImpulse,
                maxAllowedAFIMag / AFIMag
              );

            actualFrictionImpulse = ptInfo.accumulatedFrictionImpulse.subtract(
              origAccumulatedFrictionImpulse
            );

            body0.applyBodyWorldImpulse(
              actualFrictionImpulse,
              ptInfo.r0,
              false
            );
            if (body1)
              body1.applyBodyWorldImpulse(
                JNumber3D.getScaleVector(actualFrictionImpulse, -1),
                ptInfo.r1,
                false
              );
          }
  ...
  processCollisionForShock(collision, dt) {
    ...
    if (!body0.get_movable()) body0 = null;
    if (body1 && !body1.get_movable()) body1 = null;
    ...
    for (let ptInfo of collision.pointInfo) {
      ...
      if (body0) {
        normalVel =
          body0.getVelocity(ptInfo.r0).dotProduct(N) +
          body0.getVelocityAux(ptInfo.r0).dotProduct(N);
      }
      if (body1) {
        normalVel -=
          body1.getVelocity(ptInfo.r1).dotProduct(N) +
          body1.getVelocityAux(ptInfo.r1).dotProduct(N);
      }

      let finalNormalVel =
        (ptInfo.initialPenetration - JConfig.allowedPenetration) / timescale;
      if (finalNormalVel < 0) {
        continue;
      }
      ...
      ptInfo.accumulatedNormalImpulseAux = Math.max(
        ptInfo.accumulatedNormalImpulseAux + impulse,
        0
      );
      let actualImpulse = JNumber3D.getScaleVector(
        N,
        ptInfo.accumulatedNormalImpulseAux - orig
      );
      ...
      if (body1)
        body1.applyBodyWorldImpulse(
          JNumber3D.getScaleVector(actualImpulse, -1),
          ptInfo.r1,
          false
        );
    }
  ...
  sortPositionX(body0, body1) {
    if (
      body0.get_currentState().position.x < body1.get_currentState().position.x
    )
      return -1;
    else if (
      body0.get_currentState().position.x > body1.get_currentState().position.x
  ...
  sortPositionY(body0, body1) {
    if (
      body0.get_currentState().position.y < body1.get_currentState().position.y
    )
      return -1;
    else if (
      body0.get_currentState().position.y > body1.get_currentState().position.y
  ...
  sortPositionZ(body0, body1) {
    if (
      body0.get_currentState().position.z < body1.get_currentState().position.z
    )
      return -1;
    else if (
      body0.get_currentState().position.z > body1.get_currentState().position.z
  ...
  doShockStep(dt) {
    if (
      Math.abs(this._gravity.x) > Math.abs(this._gravity.y) &&
      Math.abs(this._gravity.x) > Math.abs(this._gravity.z)
    ) {
      this._bodies = this._bodies.sort(this.sortPositionX);
      this._collisionSystem.collBody = this._collisionSystem.collBody.sort(
        this.sortPositionX
      );
    } else if (
      Math.abs(this._gravity.y) > Math.abs(this._gravity.z) &&
      Math.abs(this._gravity.y) > Math.abs(this._gravity.x)
    ) {
      ...
      Math.abs(this._gravity.z) > Math.abs(this._gravity.x) &&
      Math.abs(this._gravity.z) > Math.abs(this._gravity.y)
    ) {
      ...
      if (body.get_movable()) {
        if (body.collisions.length == 0 || !body.isActive) {
          ...
          for (let info of body.collisions) {
            ...
            if (
              (body0 == body && (!body1 || !body1.get_movable())) ||
              (body1 == body && (!body0 || !body0.get_movable()))
            ) {
  ...
  handleAllConstraints(dt: number, iter: number, forceInelastic: boolean) {
    ...
    if (this._constraints.length > 0) {
      for (let constraint of this._constraints) constraint.preApply(dt);
    ...
    for (let step = 0; step < iter; step++) {
      ...
      for (let collInfo of this._collisions) {
        if (!collInfo.satisfied) {
          if (forceInelastic) flag = this.processContactFn(collInfo, dt);
          else flag = this.processCollisionFn(collInfo, dt);
    ...
    for (let body of this._activeBodies) {
      ...
      if (body_collisions.length > 0) {
        for (let collisionInfo of body_collisions) {
          ...
          if (other_body == body) other_body = collisionInfo.objInfo.body1;

          if (!other_body.isActive)
            body.addMovementActivation(
              body.get_currentState().position,
              other_body
            );
        }
      }
  ...
  updateAllController(dt) {
    for (let controller of this._controllers) controller.updateController(dt);
  }

  updateAllVelocities(dt) {
    for (let activeBody of this._activeBodies) activeBody.updateVelocity(dt);
  }

  notifyAllPostPhysics(dt) {
    for (let activeBody of this._activeBodies) activeBody.postPhysics(dt);
  }
  ...
  detectAllCollisions(dt) {
    ...
    this._collisionSystem.detectAllCollisions(
      this._activeBodies,
      this._collisions
    );

    for (let activeBody of this._activeBodies) activeBody.restoreState();
  }
  ...
  integrate(dt) {
    ...
    if (JConfig.solverType == 'ACCUMULATED') this.updateContactCache();
