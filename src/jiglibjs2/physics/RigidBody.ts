...
import { TriangleVertexIndices } from '../data/TriangleVertexIndices';
...
export class RigidBody {
  ...
  constructor(skin: ISkin3D) {
    ...
    this._invOrientation = JMatrix3D.getInverseMatrix(
      this._currState.orientation
    );
    this._linVelDamping = new Vector3D(0.999, 0.999, 0.999);
    this._rotVelDamping = new Vector3D(0.999, 0.999, 0.999);
    this._maxLinVelocities = new Vector3D(
      JMath3D.NUM_HUGE,
      JMath3D.NUM_HUGE,
      JMath3D.NUM_HUGE
    );
    this._maxRotVelocities = new Vector3D(
      JMath3D.NUM_HUGE,
      JMath3D.NUM_HUGE,
      JMath3D.NUM_HUGE
    );
  ...
  pitch(rot) {
    this.setOrientation(
      JMatrix3D.getAppendMatrix3D(
        this.get_currentState().orientation,
        JMatrix3D.getRotationMatrixAxis(rot, Vector3D.X_AXIS)
      )
    );
  }
  ...
  yaw(rot) {
    this.setOrientation(
      JMatrix3D.getAppendMatrix3D(
        this.get_currentState().orientation,
        JMatrix3D.getRotationMatrixAxis(rot, Vector3D.Y_AXIS)
      )
    );
  }
  ...
  roll(rot) {
    this.setOrientation(
      JMatrix3D.getAppendMatrix3D(
        this.get_currentState().orientation,
        JMatrix3D.getRotationMatrixAxis(rot, Vector3D.Z_AXIS)
      )
    );
  }
  ...
  applyWorldImpulse(impulse, pos, active = true) {
    ...
    this._currState.linVelocity = this._currState.linVelocity.add(
      JNumber3D.getScaleVector(impulse, this._invMass)
    );

    var rotImpulse = pos
      .subtract(this._currState.position)
      .crossProduct(impulse);
  ...
  applyWorldImpulseAux(impulse, pos, active = true) {
    ...
    this._currLinVelocityAux = this._currLinVelocityAux.add(
      JNumber3D.getScaleVector(impulse, this._invMass)
    );
  ...
  updateVelocity(dt) {
    if (!this._movable || !this.isActive) return;

    this._currState.linVelocity = this._currState.linVelocity.add(
      JNumber3D.getScaleVector(this._force, this._invMass * dt)
    );
  ...
  updatePositionWithAux(dt: number) {
    ...
    this._currState.position = this._currState.position.add(
      JNumber3D.getScaleVector(
        this._currState.linVelocity.add(this._currLinVelocityAux),
        dt
      )
    );
    ...
    if (ang > 0) {
      ...
      this._currState.orientation = JMatrix3D.getAppendMatrix3D(
        this._currState.orientation,
        rot
      );
  ...
  tryToFreeze(dt) {
    ...
    if (
      this._currState.position
        .subtract(this._lastPositionForDeactivation)
        .get_length() > JConfig.posThreshold
    ) {
      this._lastPositionForDeactivation = this._currState.position.clone();
      this._inactiveTime = 0;
      return;
    ...
    var deltaMat = JMatrix3D.getSubMatrix(
      this._currState.orientation,
      this._lastOrientationForDeactivation
    );

    var cols = JMatrix3D.getCols(deltaMat);

    if (
      cols[0].get_length() > ot ||
      cols[1].get_length() > ot ||
      cols[2].get_length() > ot
    ) {
      this._lastOrientationForDeactivation =
        this._currState.orientation.clone();
      this._inactiveTime = 0;
      return;
    }
  ...
  set_mass(m) {
    ...
    this.updateGravity(
      physicsSystem.get_gravity(),
      physicsSystem.get_gravityAxis()
    );
  }

  setInertia(matrix3D) {
    this._bodyInertia = matrix3D.clone();
    this._bodyInvInertia = JMatrix3D.getInverseMatrix(
      this._bodyInertia.clone()
    );
  ...
  updateInertia() {
    this._invOrientation = JMatrix3D.getTransposeMatrix(
      this._currState.orientation
    );
  ...
  getVelocity(relPos) {
    return this._currState.linVelocity.add(
      this._currState.rotVelocity.crossProduct(relPos)
    );
  }

  getVelocityAux(relPos) {
    return this._currLinVelocityAux.add(
      this._currRotVelocityAux.crossProduct(relPos)
    );
  }

  getShouldBeActive() {
    return (
      this._currState.linVelocity.get_length() > JConfig.velThreshold ||
      this._currState.rotVelocity.get_length() > JConfig.angVelThreshold
    );
  }
  ...
  doMovementActivations(physicsSystem) {
    if (
      ...
      this._currState.position
        .subtract(this._storedPositionForActivation)
        .get_length() < JConfig.posThreshold
    )
      return;

    for (
      var _bodiesToBeActivatedOnMovement_i = 0,
        _bodiesToBeActivatedOnMovement_l =
          this._bodiesToBeActivatedOnMovement.length,
        body;
      _bodiesToBeActivatedOnMovement_i < _bodiesToBeActivatedOnMovement_l &&
      (body =
        this._bodiesToBeActivatedOnMovement[_bodiesToBeActivatedOnMovement_i]);
      _bodiesToBeActivatedOnMovement_i++
    ) {
  ...
  addMovementActivation(pos, otherBody) {
    if (this._bodiesToBeActivatedOnMovement.indexOf(otherBody) > -1) return;
  ...
  setConstraintsAndCollisionsUnsatisfied() {
    for (
      ...
      (_constraint = this._constraints[_constraints_i]);
      _constraints_i++
    )
      _constraint.satisfied = false;

    for (
      var collisions_i = 0, collisions_l = this.collisions.length, _collision;
      collisions_i < collisions_l &&
      (_collision = this.collisions[collisions_i]);
      collisions_i++
    )
  ...
  hitTestObject3D(obj3D) {
    var num1, num2;
    num1 = this._currState.position
      .subtract(obj3D.get_currentState().position)
      .get_length();
  ...
  disableCollisions(body) {
    if (this._nonCollidables.indexOf(body) < 0) this._nonCollidables.push(body);
  }
  ...
  limitVel() {
    this._currState.linVelocity.x = JMath3D.getLimiteNumber(
      this._currState.linVelocity.x,
      -this._maxLinVelocities.x,
      this._maxLinVelocities.x
    );
    this._currState.linVelocity.y = JMath3D.getLimiteNumber(
      this._currState.linVelocity.y,
      -this._maxLinVelocities.y,
      this._maxLinVelocities.y
    );
    this._currState.linVelocity.z = JMath3D.getLimiteNumber(
      this._currState.linVelocity.z,
      -this._maxLinVelocities.z,
      this._maxLinVelocities.z
    );
  }
  ...
  limitAngVel() {
    ...
    if (f > 1)
      this._currState.rotVelocity = JNumber3D.getDivideVector(
        this._currState.rotVelocity,
        f
      );
  }
  ...
  updateObject3D() {
    if (this._skin) {
      this._skin.transform = JMatrix3D.getAppendMatrix3D(
        this._currState.orientation,
        JMatrix3D.getTranslationMatrix(
          this._currState.position.x,
          this._currState.position.y,
          this._currState.position.z
        )
      );
    }
  }
