...
export class JWheel {
  noslipVel = 2000; //0.2; // Number
  slipVel = 4000; //0.4; // Number
  ...
  getActualPos() {
    return this._pos.add(
      JNumber3D.getScaleVector(this._axisUp, this._displacement)
    );
  }
  ...
  addForcesToCar(dt) {
    ...
    this.worldPos = carBody
      .get_currentState()
      .orientation.transformVector(this._pos);
    this.worldPos = carBody.get_currentState().position.add(this.worldPos);
    this.worldAxis = carBody
      .get_currentState()
      .orientation.transformVector(this._axisUp);

    this.wheelFwd = JMatrix3D.getRotationMatrix(
      this.worldAxis.x,
      this.worldAxis.y,
      this.worldAxis.z,
      this._steerAngle
    ).transformVector(carBody.get_currentState().getOrientationCols()[2]);
    ...
    this.wheelRayEnd = this.worldPos.subtract(
      JNumber3D.getScaleVector(this.worldAxis, this._radius)
    );
    this.wheelRay = new JSegment(
      this.wheelRayEnd.add(JNumber3D.getScaleVector(this.worldAxis, rayLen)),
      JNumber3D.getScaleVector(this.worldAxis, -rayLen)
    );
    ...
    for (iRay = 0; iRay < numRays; iRay++) {
      ...
      yOffset =
        this._radius *
        (1 - Math.cos((90 * (distFwd / this._radius) * Math.PI) / 180));
      segment = segments[iRay] = this.wheelRay.clone();
      segment.origin = segment.origin.add(
        JNumber3D.getScaleVector(this.wheelFwd, distFwd).add(
          JNumber3D.getScaleVector(this.wheelUp, yOffset)
        )
      );
      if (
        this._collisionSystem.segmentIntersect(
          collOutBodyData,
          segment,
          carBody
        )
    ...
    if (numRays > 1) {
      for (iRay = 0; iRay < numRays; iRay++) {
        ...
        if (collOutBodyData.frac <= 1)
          groundNormal = groundNormal.add(
            JNumber3D.getScaleVector(
              this.worldPos.subtract(segments[iRay].getEnd()),
              1 - collOutBodyData.frac
            )
          );
      }
    ...
    var tempv = carBody
      .get_currentState()
      .orientation.transformVector(this._pos);
    this.wheelPointVel = carBody
      .get_currentState()
      .linVelocity.add(
        carBody.get_currentState().rotVelocity.crossProduct(tempv)
      );

    this.rimVel = JNumber3D.getScaleVector(
      this.wheelLeft.crossProduct(groundPos.subtract(this.worldPos)),
      this._angVel
    );
    this.wheelPointVel = this.wheelPointVel.add(this.rimVel);

    if (otherBody.get_movable()) {
      this.worldVel = otherBody
        .get_currentState()
        .linVelocity.add(
          otherBody
            .get_currentState()
            .rotVelocity.crossProduct(
              groundPos.subtract(otherBody.get_currentState().position)
            )
        );
    ...
    this.wheelCentreVel = carBody
      .get_currentState()
      .linVelocity.add(
        carBody.get_currentState().rotVelocity.crossProduct(tempv)
      );
    this._angVelForGrip =
      this.wheelCentreVel.dotProduct(this.groundFwd) / this._radius;
    ...
    if (otherBody.get_movable()) {
      ...
      if (force.get_lengthSquared() > maxOtherBodyForce * maxOtherBodyForce) {
        force = JNumber3D.getScaleVector(
          force,
          maxOtherBodyForce / force.get_length()
        );
      }
      otherBody.addWorldForce(
        JNumber3D.getScaleVector(force, -1),
        groundPos,
        false
      );
    }
  ...
  update(dt) {
    ...
    this._upSpeed =
      (this._displacement - this._lastDisplacement) /
      Math.max(dt, JMath3D.NUM_TINY);
    ...
    if (this._locked) {
      ...
      if (
        (origAngVel > this._angVelForGrip &&
