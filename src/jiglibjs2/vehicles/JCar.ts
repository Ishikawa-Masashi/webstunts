...
import { JChassis } from './JChassis';
import { JWheel } from './JWheel';
...
export class JCar {
  ...
  setupWheel(
    _name,
    pos,
    wheelSideFriction = 2000000,
    wheelFwdFriction = 2000000,
    wheelTravel = 3,
    wheelRadius = 10,
  ...
  addExternalForces(dt) {
    for (
      var wheels_i = 0, wheels_l = this.get_wheels().length, wheel;
      wheels_i < wheels_l && (wheel = this.get_wheels()[wheels_i]);
      wheels_i++
    ) {
