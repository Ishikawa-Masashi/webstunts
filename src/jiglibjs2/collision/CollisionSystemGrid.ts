...
import { CollisionSystemAbstract } from './CollisionSystemAbstract';
import { CollisionSystemGridEntry } from './CollisionSystemGridEntry';
...
export class CollisionSystemGrid extends CollisionSystemAbstract {
  ...
  calcGridForSkin3(colBody) {
    ...
    min.x = JMath3D.getLimiteNumber(
      min.x,
      this.startPoint.x,
      this.startPoint.x + this.sizeX
    );
    min.y = JMath3D.getLimiteNumber(
      min.y,
      this.startPoint.y,
      this.startPoint.y + this.sizeY
    );
    min.z = JMath3D.getLimiteNumber(
      min.z,
      this.startPoint.z,
      this.startPoint.z + this.sizeZ
    );
  ...
  calcGridForSkin6(colBody) {
    ...
    if (sides.x > this.dx || sides.y > this.dy || sides.z > this.dz) {
      ...
      tempStoreObject.j = j;
      tempStoreObject.k = k;
      tempStoreObject.fi = fi;
      tempStoreObject.fj = fj;
      tempStoreObject.fk = fk;
  ...
  addCollisionBody(body) {
    if (this.collBody.indexOf(body) < 0) this.collBody.push(body);
  ...
  removeAllCollisionBodies() {
    for (
      var collBody_i = 0, collBody_l = this.collBody.length, body;
      collBody_i < collBody_l && (body = this.collBody[collBody_i]);
      collBody_i++
    ) {
      if (body.externalData != null) {
        body.externalData.collisionBody = null;
  ...
  collisionSkinMoved(colBody) {
    ...
    var start;
    //if (gridIndex >= 0**)
    if (this.gridEntries.length - 1 > gridIndex && gridIndex >= 0)
      // check if it's outside the gridspace, if so add to overflow
      start = this.gridEntries[gridIndex];
    else start = this.overflowEntries;
  ...
  getListsToCheck(colBody) {
    ...
    j = tempStoreObject.j;
    k = tempStoreObject.k;
    fi = tempStoreObject.fi;
    fj = tempStoreObject.fj;
    fk = tempStoreObject.fk;
    ...
    var maxI = 1,
      maxJ = 1,
      maxK = 1;
    if (fi + delta.x / this.dx < 1) maxI = 0;
    if (fj + delta.y / this.dy < 1) maxJ = 0;
    if (fk + delta.z / this.dz < 1) maxK = 0;
  ...
  detectAllCollisions(bodies, collArr) {
    ...
    for (
      var bodies_i = 0, bodies_l = bodies.length, body;
      bodies_i < bodies_l && (body = bodies[bodies_i]);
      bodies_i++
    ) {
      ...
      for (
        var lists_i = 0, lists_l = lists.length, entry;
        lists_i < lists_l && (entry = lists[lists_i]);
        lists_i++
      ) {
        for (entry = entry.next; entry != null; entry = entry.next) {
          ...
          if (
            entry.collisionBody.isActive &&
            bodyID > entry.collisionBody.get_id()
          )
            continue;

          if (
            this.checkCollidables(body, entry.collisionBody) &&
            this.detectionFunctors[
              bodyType + '_' + entry.collisionBody.get_type()
            ] != undefined
          ) {
            ...
            fu =
              this.detectionFunctors[
                info.body0.get_type() + '_' + info.body1.get_type()
              ];
