...
import { CollDetectCapsuleBox } from './CollDetectCapsuleBox';
import { CollDetectCapsuleCapsule } from './CollDetectCapsuleCapsule';
import { CollDetectCapsulePlane } from './CollDetectCapsulePlane';
import { CollDetectCapsuleTerrain } from './CollDetectCapsuleTerrain';
...
import { CollDetectSphereCapsule } from './CollDetectSphereCapsule';
...
export class CollisionSystemAbstract {
  ...
  addCollisionBody(body: RigidBody) {
    if (this.collBody.indexOf(body) < 0) this.collBody.push(body);
  }

  removeCollisionBody(body: RigidBody) {
  ...
  detectCollisions(body: RigidBody, collArr) {
    ...
    for (let _collBody of this.collBody) {
      ...
      if (
        this.checkCollidables(body, _collBody) &&
        this.detectionFunctors[body.get_type() + '_' + _collBody.get_type()] !=
          undefined
      ) {
  ...
  segmentIntersect(out: CollOutBodyData, seg: JSegment, ownerBody: RigidBody) {
    ...
    for (let _collBody of this.collBody) {
      if (_collBody != ownerBody && this.segmentBounding(seg, _collBody)) {
        if (
          _collBody.segmentIntersect(obj, seg, _collBody.get_currentState())
        ) {
  ...
  checkCollidables(body0: RigidBody, body1: RigidBody) {
    if (
      body0.get_nonCollidables().length == 0 &&
      body1.get_nonCollidables().length == 0
    )
      return true;
