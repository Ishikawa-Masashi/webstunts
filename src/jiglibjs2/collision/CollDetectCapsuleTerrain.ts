...
export class CollDetectCapsuleTerrain extends CollDetectFunctor {
  ...
  public collDetect(info, collArr) {
    ...
    if (
      Math.min(obj1.height, obj2.height) <
      JConfig.collToll + capsule.get_radius()
    ) {
      var oldDepth = capsule.get_radius() - obj1.height;
      var worldPos = pos1.subtract(
        JNumber3D.getScaleVector(obj2.normal, capsule.get_radius())
      );
    ...
    if (collPts.length > 0) {
      ...
      mat.friction =
        0.5 *
        (capsule.get_material().friction + terrain.get_material().friction);
      collInfo.mat = mat;
