...
export class CollDetectBoxPlane extends CollDetectFunctor {
  ...
  collDetect(info, collArr) {
    ...
    if (centreDist > box.get_boundingSphere() + JConfig.collToll) return;
    ...
    if (collPts.length > 0) {
      ...
      mat.restitution =
        0.5 *
        (box.get_material().restitution + plane.get_material().restitution);
      mat.friction =
        0.5 * (box.get_material().friction + plane.get_material().friction);
      collInfo.mat = mat;
