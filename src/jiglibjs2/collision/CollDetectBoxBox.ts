...
export class CollDetectBoxBox extends CollDetectFunctor {
  ...
  addPoint(contactPoints, pt, combinationDistanceSq) {
    for (
      var contactPoints_i = 0,
        contactPoints_l = contactPoints.length,
        contactPoint;
      contactPoints_i < contactPoints_l &&
      (contactPoint = contactPoints[contactPoints_i]);
      contactPoints_i++
    ) {
      if (
  ...
  getSupportPoint(box, axis) {
    ...
    var _as = axis.dotProduct(orientationCol[0]),
      _au = axis.dotProduct(orientationCol[1]),
      _ad = axis.dotProduct(orientationCol[2]),
      tiny = JMath3D.NUM_TINY;
  ...
  getAABox2EdgeIntersectionPoints(
    contactPoint,
    origBoxSides,
    origBoxState,
    edgePt0,
    ...
    edgeDir.normalize();
    var ptArr, faceOffsets: any, edgePt0Arr, edgePt1Arr, edgeDirArr, sidesArr;
    ...
    for (iDir = 2; iDir >= 0; iDir--) {
      ...
      for (iFace = 1; iFace >= 0; iFace--) {
        ...
        if (frac >= 0) {
          pt = JNumber3D.getScaleVector(edgePt0, 1 - frac).add(
            JNumber3D.getScaleVector(edgePt1, frac)
          );
          ptArr = JNumber3D.toArray(pt);
          if (
            ptArr[jDir] > -sidesArr[jDir] - tiny &&
            ptArr[jDir] < sidesArr[jDir] + tiny &&
            ptArr[kDir] > -sidesArr[kDir] - tiny &&
            ptArr[kDir] < sidesArr[kDir] + tiny
          ) {
  ...
  getBox2BoxEdgesIntersectionPoints(contactPoint, box0, box1, newState) {
    ...
    var edgePt0, edgePt1;
    for (
      var boxEdges_i = 0, boxEdges_l = boxEdges.length, boxEdge;
      boxEdges_i < boxEdges_l && (boxEdge = boxEdges[boxEdges_i]);
      boxEdges_i++
    ) {
      edgePt0 = boxPts[boxEdge.ind0];
      edgePt1 = boxPts[boxEdge.ind1];
      num += this.getAABox2EdgeIntersectionPoints(
        contactPoint,
        box0.get_sideLengths(),
        box0State,
        edgePt0,
        edgePt1
      );
  ...
  collDetect(info, collArr) {
    ...
    if (!box0.get_boundingBox().overlapTest(box1.get_boundingBox())) return;

    var numTiny = JMath3D.NUM_TINY,
      numHuge = JMath3D.NUM_HUGE;
    ...
    if (
      box1
        .get_currentState()
        .position.subtract(box0.get_currentState().position)
        .dotProduct(N) > 0
    )
      N.negate();
    ...
    var contactPoints = [];
    this.combinationDist =
      0.05 *
      Math.min(
        Math.min(
          box0.get_sideLengths().x,
          box0.get_sideLengths().y,
          box0.get_sideLengths().z
        ),
        Math.min(
          box1.get_sideLengths().x,
          box1.get_sideLengths().y,
          box1.get_sideLengths().z
        )
      );
    ...
    var bodyDelta = box0
      .get_currentState()
      .position.subtract(box0.get_oldState().position)
      .subtract(
        box1.get_currentState().position.subtract(box1.get_oldState().position)
      );
    ...
    switch (minAxis) {
      ...
      case 14: {
        ...
        //-----------------------------------------------------------------
        if (Math.abs(div) < numTiny) return;
    ...
    if (contactPoints.length > 0) {
      ...
      if (maxDist < minDist + numTiny) maxDist = minDist + numTiny;
    ...
    mat.restitution =
      0.5 * (box0.get_material().restitution + box1.get_material().restitution);
    mat.friction =
      0.5 * (box0.get_material().friction + box1.get_material().friction);
    collInfo.mat = mat;
