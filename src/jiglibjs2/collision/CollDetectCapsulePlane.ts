...
export class CollDetectCapsulePlane extends CollDetectFunctor {
  ...
  collDetect(info, collArr) {
    ...
    if (Math.min(oldDist, newDist) < capsule.get_radius() + JConfig.collToll) {
      ...
      var worldPos = oldPos.subtract(
        JNumber3D.getScaleVector(plane.get_normal(), capsule.get_radius())
      );
    ...
    if (collPts.length > 0) {
      ...
      mat.restitution =
        0.5 *
        (capsule.get_material().restitution + plane.get_material().restitution);
      mat.friction =
        0.5 * (capsule.get_material().friction + plane.get_material().friction);
      collInfo.mat = mat;
