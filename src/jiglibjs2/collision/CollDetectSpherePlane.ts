...
export class CollDetectSpherePlane extends CollDetectFunctor {
  ...
  collDetect(info, collArr) {
    ...
    if (
      Math.min(newDist, oldDist) >
      sphere.get_boundingSphere() + JConfig.collToll
    ) {
    ...
    var worldPos = sphere
      .get_oldState()
      .position.subtract(
        JNumber3D.getScaleVector(plane.get_normal(), sphere.get_radius())
      );
    ...
    mat.restitution =
      0.5 *
      (sphere.get_material().restitution + plane.get_material().restitution);
    mat.friction =
      0.5 * (sphere.get_material().friction + plane.get_material().friction);
    collInfo.mat = mat;
