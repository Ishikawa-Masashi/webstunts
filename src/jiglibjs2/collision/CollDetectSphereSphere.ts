...
export class CollDetectSphereSphere extends CollDetectFunctor {
  ...
  collDetect(info, collArr) {
    ...
    var oldDelta = sphere0
      .get_oldState()
      .position.subtract(sphere1.get_oldState().position);
    var newDelta = sphere0
      .get_currentState()
      .position.subtract(sphere1.get_currentState().position);
    ...
    if (
      ...
      if (oldDist > JMath3D.NUM_TINY) {
        ...
        oldDelta = JMatrix3D.getRotationMatrix(
          0,
          0,
          1,
          360 * Math.random()
      ...
      var worldPos = sphere1
        .get_oldState()
        .position.add(
          JNumber3D.getScaleVector(oldDelta, sphere1.get_radius() - 0.5 * depth)
        );
      ...
      mat.restitution =
        0.5 *
        (sphere0.get_material().restitution +
          sphere1.get_material().restitution);
      mat.friction =
        0.5 *
        (sphere0.get_material().friction + sphere1.get_material().friction);
      collInfo.mat = mat;
